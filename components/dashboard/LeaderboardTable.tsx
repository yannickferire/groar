...
const gridColsCompact = "grid-cols-[2.5rem_1fr_auto] md:grid-cols-[3rem_1fr_5rem_4.5rem]";

export default function LeaderboardTable({ apiUrl = "/api/leaderboard", currentUserId, initialData, compact }: LeaderboardTableProps) {
    ...
    <div>
      ...
      {loading ? (
        <div className="rounded-2xl border-fade overflow-hidden">
          <div className={`grid ${compact ? gridColsCompact : gridCols} gap-3 md:gap-4 px-4 md:px-5 py-3 bg-sidebar text-xs text-muted-foreground font-medium uppercase tracking-wide`}>
            <span>#</span>
            <span>User</span>
            {!compact && <span className="text-right hidden md:block">Streak</span>}
            <span className="text-right hidden md:block">Exports</span>
            {!compact && <span className="text-right hidden md:block">Today</span>}
            <span className="text-right">Score</span>
          </div>
          {[...Array(5)].map((_, i) => (
            <div key={i} className={`grid ${compact ? gridColsCompact : gridCols} gap-3 md:gap-4 px-4 md:px-5 py-3 border-t border-border/50`}>
              ...
              {!compact && <div className="h-5 w-8 rounded bg-sidebar ml-auto hidden md:block" />}
        ...
        <div className="rounded-2xl border-fade overflow-hidden">
          ...
          {leaderboard.map((user, index) => {
            ...
            return (
              <div
                key={user.id || index}
                className={`grid ${compact ? gridColsCompact : gridCols} gap-3 md:gap-4 px-4 md:px-5 py-3 border-t border-border/50 items-center ${
